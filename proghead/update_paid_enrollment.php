...
if (isset($_GET['lrn'])) {
    ...
    if ($retrieveDataResult && $retrieveDataResult->num_rows > 0) {
        ...
        // Update the "enrolled" column in the enrollment table
        $updateEnrollmentQuery = "UPDATE enrollment SET enrolled='Yes' WHERE lrn='$lrn'";
        $updateEnrollmentResult = $connection->query($updateEnrollmentQuery);

        // Update the "enrollment" column in the students table
        $updateStudentsQuery = "UPDATE students SET enrollment='YES' WHERE lrn='$lrn'";
        $updateStudentsResult = $connection->query($updateStudentsQuery);

        if ($updateEnrollmentResult && $updateStudentsResult) {
            // Send message using Semaphore API
            $semaphoreApiKey = '32e4b035c774155ebc33d32581cbdc95';

            // Message to be sent
            $message = "This is Lord Jesus Blessed Academy, this is to inform you that $fname $mname $lname is officially enrolled!";
